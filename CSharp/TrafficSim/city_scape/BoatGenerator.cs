...
public class BoatGenerator: Conveyor
{
	private double x, y;
	...
	public BoatGenerator(double new_x, double new_y, int rate, Scene scene): base(scene)
	{
		x = new_x;
		y = new_y;
		this.rate = rate;
	...
	public override void Update()
	{
		if(timer == 0)
		{
			...
			if (rd <= 0.5)
				new_mobile = new BigBoat(x, y, this.scene);
			else
				new_mobile = new Boat(x, y, this.scene);
