...
// A Merge is a 1 dimensional Conveyor which connects two incoming
// Conveyors to 1 outgoing Conveyor.  The Merge decides which
// incoming Mobile to let enter the merge intersection and controls
// the other incoming Mobile objects so that no collisions occur.
// The merge_space of a Merge is the amount of distance before the Merge
// that the Mobile objects are kept while the intersection is occupied.
// If the merge intersection is clear (no traffic) and only one incoming
...
// merge_space of the merge, then only one of these Mobile objects is
...
// in spaceRear(Conveyor) since the Merge will want to know which of
// its incoming Conveyors is asking if there is space.  Depending on
// the situation, the Merge can report to the Conveyor that there is
...
public class Merge: Conveyor
{
	...
	public Merge(double new_x, double new_y, double new_merge_space, Scene scene): base(scene)
	{
		x = new_x;
		y = new_y;
		in1 = null;
	...
	public Merge(Conveyor new_in1, Conveyor new_in2, Conveyor new_out,
