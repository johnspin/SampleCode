// AccidentRouter class
using System;
using System.Drawing;

// An AccidentRouter connects one conveyor to two conveyors

public class AccidentRouter: Conveyor
{
	...
	// Construct a new AccidentRouter at coordinates (new_x, new_y).
	public AccidentRouter(double new_x, double new_y, Scene scene): base(scene)
	{
		x = new_x;
		y = new_y;
		cin = null;
	...
	public AccidentRouter(Conveyor new_in, Conveyor new_out1, Conveyor new_out2, Scene scene): base(scene)
	{
		cin = new_in;
		out1 = new_out1;
		out2 = new_out2;
		...
		if(cin.OutX() != out1.InX())
			throw new ArgumentException("AccidentRouter: cin.OutX() != out1.InX()");
		if(cin.OutY() != out1.InY())
			throw new ArgumentException("AccidentRouter: cin.OutY() != out1.InY()");
		if(cin.OutX() != out2.InX())
			throw new ArgumentException("AccidentRouter: cin.OutX() != out2.InX()");
		if(cin.OutY() != out2.InY())
			throw new ArgumentException("AccidentRouter: in.OutY() != out2.InY()");
	...
	// Add an outgoing conveyor to the AccidentRouter
	...
	{
		if (mobile is Suv && (out1 is InvRoad || out2 is InvRoad))	
		{
			if (out1 is InvRoad)
